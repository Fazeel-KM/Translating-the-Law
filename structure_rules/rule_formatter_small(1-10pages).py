...
for input_folder, output_dir in zip(input_folders, output_dirs):
    ...
    for pdf_file_path in pdf_files:
        ...
        # Extract text from the 'markdown' key in each page
        print("Extracting text from OCR response...")
        try:
            if 'pages' in response_dict:
                extracted_text = " ".join(page.get('markdown', '') for page in response_dict['pages'])
            else:
        ...
        # Create an exhaustive prompt for DeepSeek API
        print("Creating exhaustive prompt for DeepSeek API...")
        prompt = f"""
...
"""
        ...
        # Call DeepSeek API to generate structured JSON
        print("Calling DeepSeek API to generate JSON...")
        ...
        if not response:
            print(f"Failed to generate JSON for {pdf_file_path.name}. Skipping.")
            continue
        alpaca_json = response.choices[0].message.content
        print("DeepSeek API response received.")

        # Save the structured JSON to file
        print("Saving structured JSON to file...")
        output_file_path = output_dir / f"{pdf_file_path.stem}_alpaca.json"
        try:
            ...
            print("Failed to parse Alpaca JSON, saving raw response")
            with open(output_file_path, "w", encoding="utf-8") as output_file:
